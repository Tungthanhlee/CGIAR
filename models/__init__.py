...
def train_loop(_print, cfg, model, train_loader,valid_loader, criterion, valid_criterion, optimizer, scheduler, start_epoch, best_metric):
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        VALID
        """
        top_lloss = valid_model(_print, cfg, model, valid_criterion, valid_loader)
...
def swa_train_loop(_print, cfg, model, swa_model, train_loader,valid_loader, criterion, valid_criterion, optimizer, scheduler, start_epoch, best_metric):
    delay_swa = 0
    swa_n = -1
    
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        train_loop(_print, cfg, model, \
                train_loader, valid_loader, criterion, valid_criterion,\
                optimizer, scheduler, start_epoch, best_metric)
        current_metric = valid_model(_print, cfg, model, valid_criterion,valid_loader, tta=False)
